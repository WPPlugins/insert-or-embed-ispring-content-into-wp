...
#********************************************************************************************************************************
function ispring_print_page_navi($num_records)
{
				...
				if ($num_pages != 1)
				{
					for ($i = 1; $i <= $num_pages; ++$i)
					{
						...
						{
							echo "<a href=\"media-upload.php?type=ispring_upload&tab=ispring&npage=$i\">$i</a>";
...
function ispring_print_detail_form($num, $tab="upload", $file_url="", $dirname="")
{
...
<?php
}#end ispring_print_detail_form()


function ispring_printInsertForm()
{
//echo "<h3>Start ispring_printInsertForm</h3>";
	$dirs = ispring_getDirs();
	if (count($dirs)>0)
	{
	ispring_print_js("ispring");
	?>
	<title>Media Library</title>
	<?php
	 $uploadDirUrl=ispring_getUploadsUrl();
	 //START PAGIGNATION
	 ?>
	 <div style="text-align:right; padding:5px; padding-right:10px; margin:5px 20px;"> 
	 <?php  $bound= ispring_print_page_navi(count($dirs)); // print the pagignation and return upper and lower bound ?>
	 </div>
	 <?php
		...
		echo "<table class='widefat'>";
			foreach ($dirs as $i=>$dir):
				...
				echo '<tr id="content_item_'.$i.'" class="'; if ($i % 2 == 1)echo 'alternate '; echo 'iedit">
						...
						echo '</span>';
						echo '</div>';
						ispring_print_detail_form($i, "ispring" , $uploadDirUrl.$dir."/".$file, $dir);
						echo '
						</td>
					 </tr>';
...
//echo "<h3>End ispring_printInsertForm</h3>";
}

function ispring_getUploadsPath()
{
$dir = wp_upload_dir();
return ($dir['basedir'] . "/".WP_ISPRING_EMBEDER_UPLOADS_DIR_NAME."/");
}
function ispring_getUploadsUrl()
{
$dir = wp_upload_dir();
return $dir['baseurl'] . "/".WP_ISPRING_EMBEDER_UPLOADS_DIR_NAME."/";
}
function ispring_getPluginUrl()
{
//return WP_PLUGIN_URL."/insert-or-embed-ispring-content-into-wordpress/"; #oneTarek says: This line is wrong because you are unable to rename the plugin directory
...
function ispring_getDirs()
{
$myDirectory = opendir(ispring_getUploadsPath());
$dirArray = array();
$i=0;
// get each entry
while($entryName = readdir($myDirectory)) {
	if ($entryName != "." && $entryName !=".." && is_dir(ispring_getUploadsPath().$entryName)):
	$dirArray[$i]['dir'] = $entryName;
	// store the filenames - need to iterate to get story.html or player.html
	$dirArray[$i]['file'] = ispring_getFile(ispring_getUploadsPath().$entryName);
	$i++;
	endif;
}
...
while($entryName = readdir($myDirectory)) {
	...
	{
	$f = ispring_getUploadsPath().$entryName;
	...
	// need to check the filename and only return index.html
	if (($fname == "index") && $ext == "html"):
	closedir($myDirectory);
	return $entryName;
	endif;
	}
...
function ispring_print_js($tab="upload") #added by oneTarek
{
wp_enqueue_script("jquery");
wp_enqueue_style( 'ispring_regstylecss', plugins_url('/css/style.css', __FILE__ ));
wp_register_script('ispring_jqueryformjs', WP_ISPRING_EMBEDER_PLUGIN_URL.'js/jquery.form.js');
wp_enqueue_script( 'ispring_colorboxjs', plugins_url('/js/jquery.form.js', __FILE__ ));
			...
			success : function(data) {
			 ...
			 {	
				  if (data[0] == "uploaded")
				  {
					...
					<?php }?>
					show_detail(1);
					//win.send_to_editor("[ispring_loader width='100%' height='600' frameborder='0' src='"+dir+"']");
...
function add_to_post(number)
{
	<?php if($tab=="upload"){?>
	  ...
	  {
	  ispring_rename_dir(old_name, new_name);
	  }
	  else
	  {
...
{
<?php $opt=get_ispring_embeder_options();?>
...
//alert(insert_as);
	...
	shortCode="[ispring_iframe_loader type='"+shortCodeType+"' " +shortCodeOptions+"]";
	win.send_to_editor(shortCode);

}// end insert_into_post()

function ispring_rename_dir(old_name, new_name)
{

var loading_text='<img src="<?php echo WP_ISPRING_EMBEDER_PLUGIN_URL;?>loading_16x16.gif" alt="Loading" /> Saving....';
	jQuery('#insert_msg_1').html(loading_text);	
		
			
			jQuery.getJSON("<?PHP bloginfo('url') ?>/wp-admin/admin-ajax.php?action=ispring_rename_dir&dir_name="+old_name+"&title="+new_name, function(data) {
				...
				if(data[0]=="success")
				{
				 ...
				 jQuery('#file_url_1').val("<?php echo ispring_getUploadsUrl();?>"+new_renamed_dir_name+"/"+old_file_name);
...
function delete_dir(number)
{
	...
	var loading_image='&nbsp;&nbsp;<img src="<?php echo WP_ISPRING_EMBEDER_PLUGIN_URL;?>loading_16x16.gif" alt="Launch Presentation" />&nbsp;Deleting..'
	var loading_text='<img src="<?php echo WP_ISPRING_EMBEDER_PLUGIN_URL;?>loading_16x16.gif" alt="Loading" /> Deleting....';
	...
	{			
		...
		jQuery.post("admin-ajax.php",{dir : dir_name,action:'ispring_del_dir'},function(data){
			//alert("Deleted");
			<?php if($tab=="upload"){?>
...
<?php
}#end function ispring_print_js()

function ispring_print_upload()
{
// echo "<h3>Start ispring_print_upload</h3>";
ispring_print_js();
?>
<?PHP 
...
if (count($dirs)>1)
{
echo '<span style="color:#ff0000; font-size:15px;">The trial version only supports two uploads. Please purchase the full version with unlimited uploads at <a href="http://www.elearningplugins.com/products/insert-embed-ispring-content-wordpress-plugin/" target="_blank">www.elearningplugins.com</a></span>';	
}
else
{	
?>
<form enctype="multipart/form-data" id="myForm1" action="admin-ajax.php" method="POST">
<input type="hidden" name="action" value="ispring_upload" />
...
<p><i>Please choose a .zip file that you published with the iSpring software | <a href="http://www.elearningplugins.com/increase-maximum-upload-file-size/" target="_blank">Maximum upload file size</a>: <strong><?php echo $upload_size_unit; echo $sizes[$u]; ?></strong></i></p>
<img id="media_loading" style='display:none;' src= "<?php echo ispring_getPluginUrl() . "loading.gif" ;?>" /><br />
<?php 
 	ispring_print_detail_form(1);
}
?>


<p><b>Need help?  See the screencast below:</b></p>
<iframe src="https://www.youtube.com/embed/OW73OvgLaWI" width="600" height="366" frameborder="0"></iframe>
<p/>
<p/>
<iframe src="http://www.elearningplugins.com/wordpresspluginlatestpremiumispringtrial.html" width="600px" frameborder="0">
</iframe><p/>


<?php
 //echo "<h3>END ispring_print_upload</h3>";
}



function wp_ajax_ispring_del_dir()
{
$dir = ispring_getUploadsPath().$_POST['dir'];
ispring_rrmdir($dir);
die();
}


function wp_ajax_ispring_rename_dir()
{
$arr=array();
	if(isset($_REQUEST['dir_name']) && $_REQUEST['dir_name']!="")
	{
	$target = ispring_getUploadsPath().$_REQUEST['dir_name'];
		if(file_exists($target))
		{
			...
			$title=trim($_REQUEST['title']);
				...
				{   
					...
					$new_file=ispring_getUploadsPath().$title;
...
function wp_ajax_ispring_upload()
{
$arr = array();
...
$target = ispring_getUploadsPath().$dir[0];
$index = count($dir) -1;
	if (!isset($dir[$index]) || $dir[$index] != "zip")
	...
	else
		{
			...
			if (!empty($file))
			$arr = ispring_extractZip($file,$target,$dir[0]);
			else
...
function ispring_extractZip($fileName,$target,$dir)
{
		 $arr = array();
 $zip = new ZipArchive;
     ...
     if ($res === TRUE) {
         ...
         $zip->close();
		 $file = ispring_getFile($target);

		if($file)
		{	
		 $arr[0] = 'uploaded'; 
		 $arr[1] = ispring_getUploadsUrl().$dir."/".$file;
		 $arr[2] = $dir;
		 $arr[3] =$file;
		}
		 else
			 {
			 $arr[0] ="Please upload iSpring content or verify that the content was properly zipped"; 
			 ispring_rrmdir($target);
			 }
...
function ispring_rrmdir($dir) {
   if (is_dir($dir)) {
     $objects = scandir($dir);
     foreach ($objects as $object) {
       if ($object != "." && $object != "..") {
         if (filetype($dir."/".$object) == "dir") ispring_rrmdir($dir."/".$object); else unlink($dir."/".$object);
       }
     }
     reset($objects);
     rmdir($dir);
...
function ispring_embeder_wp_head()
{
$opt=get_ispring_embeder_options();
global $ispring_embeder_size_opt;
global $ispring_embeder_width;
global $ispring_embeder_height;
global $ispring_embeder_scrollbar;
if(isset($ispring_embeder_size_opt))
	{
	if($ispring_embeder_size_opt=="custom_form_dashboard"){$ispring_embeder_width=$opt['width'].$opt['width_type']; $ispring_embeder_height=$opt['height'].$opt['height_type'];}
	}
...
?>
	<!--nivo lite box-->
	<?php if($ispring_embeder_size_opt!="lightebox_default"){ ?>
	<style type="text/css">
	.nivo-lightbox-content {
	width: <?php echo $ispring_embeder_width; ?>;
	height: <?php echo $ispring_embeder_height; ?>;
	margin:auto;
	}
		<?php if(isset($ispring_embeder_scrollbar) && $ispring_embeder_scrollbar=="no"){?>
	...
	<?php }?>
	<!--<script src="<?php echo WP_ISPRING_EMBEDER_PLUGIN_URL?>nivo-lightbox/nivo-lightbox.min.js"></script>-->
	<!--end nivo lite box-->	
<?php } else{
	global $ispring_embeder_colorbox_theme; if(!isset($ispring_embeder_colorbox_theme) || $ispring_embeder_colorbox_theme==""){$ispring_embeder_colorbox_theme=$opt["colorbox_theme"];}
	if($ispring_embeder_size_opt!="custom_form_dashboard" && $ispring_embeder_size_opt!="custom"){ $ispring_embeder_width="80%"; $ispring_embeder_height="80%";}//default
?>
...
function ispring_embeder_get_colorbox_themes()
{
	$themes=array(
...
function ispring_embeder_admin_scripts()
 {
	 if (isset($_GET['page']) && $_GET['page'] == 'ispring_content')
	 {
	 wp_enqueue_script('jquery');
	 wp_enqueue_media();
	 wp_register_script('ispring_embeder_upload', WP_ISPRING_EMBEDER_PLUGIN_URL.'js/admin.js');
	 ...
	 wp_enqueue_script('ispring_embeder_upload');
	 }
 }
 add_action( 'admin_enqueue_scripts', 'ispring_embeder_admin_scripts' );
?>